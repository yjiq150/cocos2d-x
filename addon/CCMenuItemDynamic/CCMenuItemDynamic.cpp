...
CCMenuItemSprite * CCMenuItemDynamic::dynamicItemWithTTFLabel(const char* text,  const char* font, int aSize, const CCSize &minSize,  
                                                              ...
                                                              const CCRect& centerRegion, GLubyte opacity) 
{
     ...
     CCLabelTTF *label = CCLabelTTF ::labelWithString(text ,font, aSize);
      ...
      {
          // When selectedImage is not specified -> use dimmed sprite for selectedImage
          selectedSprite = CCScale9Sprite::scale9SpriteWithFile(normalImage, centerRegion);
          ((CCScale9Sprite*)selectedSprite)->setColor(ccGRAY);
...
CCMenuItemSprite * CCMenuItemDynamic::dynamicItemWithBMFontLabel(const char* text,  const char* font, const CCSize &minSize,  const char*normalImage, const char *selectedImage, CCObject* target, SEL_MenuHandler selector, const CCRect& centerRegion, GLubyte opacity) 
{
    //Create the label
    CCLabelBMFont *label = CCLabelBMFont ::labelWithString(text ,font);
